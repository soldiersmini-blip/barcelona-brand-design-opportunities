...
appSource = appSource
  .replace(/\ninitStats\(\);\nrenderPriority\(\);\nrenderResults\(\);\s*$/, "\n")
  .concat(
    "\nglobalThis.siteTest = { allData, dedupedData, latestRoundSection, latestRoundItems, priorityItems, CURATED, SCORE_LANGUAGE_RISK_OVERRIDES, state, MY_OPPORTUNITY_IDS, MY_OPPORTUNITY_SET, AUDITED_ORDER_INDEX, getStatusSummary, directionKey, languageInfo, applicationLanguagePath, scoreLanguageRisk, roleLabels, companyLabel, locationLabel, sourceGroup, locationBucket, isActionableLink, toLinks, isChineseRelevant, isResearchOnly, isTargetOpportunity, isInternshipRole, hasLowPayRisk, riskFlags, hasOpaqueEmployerRisk, isFormalRole, isFreelanceRole, hasKnownCompensation, laborConditionInfo, experienceInfo, applicationStatus, isClosedLibraryRecord, isReviewLibraryRecord, personalMatchScore, personalFitBadge, displayedScore, postedTimestamp, rankingScore, confidenceScore, sortRecords, matchesPreset, baseRecords, matchesFilters, progressKey, progressValue, saveProgressValue, identityKey, englishOutreachText };\n",
  );
...
check((indexHtml.match(/data-quick-view=/g) || []).length === 7, "Round 77: the simplified board must expose seven complete quick views including zero-score backup");
check(/class="personal-filter"[^>]*hidden/.test(indexHtml), "Round 72: legacy personal-filter panel must stay hidden");
check(/class="scope-switcher"[^>]*hidden/.test(indexHtml), "Round 72: legacy scope switcher must stay hidden");
check(/class="filters" hidden/.test(indexHtml), "Round 72: legacy advanced filters must stay hidden");
check(/class="progress-filter-bar"[^>]*hidden/.test(indexHtml), "Round 72: legacy progress-filter row must stay hidden");
check(/class="source-tabs"[^>]*hidden/.test(indexHtml), "Round 72: legacy source-tab row must stay hidden");

check(/state\.quickView === "chineseCurrent" && !isChineseRelevant\(item\)/.test(appSource), "Round 72: Chinese-current quick view lost its relevance gate");
check(/state\.quickView === "barcelona" && locationBucket\(item\) !== "barcelona"/.test(appSource), "Round 72: Barcelona quick view lost its exact location gate");
check(/state\.quickView === "remote" && locationBucket\(item\) !== "remote"/.test(appSource), "Round 72: remote quick view lost its exact location gate");
const quickCurrentRecords = test.dedupedData.filter(
  (item) => test.MY_OPPORTUNITY_SET.has(Number(item.id)) && test.applicationStatus(item).key !== "closed",
);
const quickActionableRecords = quickCurrentRecords.filter((item) => test.displayedScore(item) > 0);
const quickBackupRecords = quickCurrentRecords.filter((item) => test.displayedScore(item) <= 0);
const quickViewCounts = {
  current: quickActionableRecords.length,
  backup: quickBackupRecords.length,
  chineseCurrent: quickCurrentRecords.filter(test.isChineseRelevant).length,
  barcelona: quickActionableRecords.filter((item) => test.locationBucket(item) === "barcelona").length,
  remote: quickActionableRecords.filter((item) => test.locationBucket(item) === "remote").length,
  history: test.dedupedData.filter((item) => test.applicationStatus(item).key === "closed").length,
};
check(
  quickViewCounts.current + quickViewCounts.backup === 211 &&
    quickViewCounts.current > 0 &&
    quickViewCounts.backup > 0 &&
    quickViewCounts.chineseCurrent === 5 &&
    quickViewCounts.barcelona + quickViewCounts.remote === quickViewCounts.current &&
    quickViewCounts.history === 629,
  `Round 77: action/backup quick-view ledgers diverged ${JSON.stringify(quickViewCounts)}`,
);
check(indexHtml.includes('data-quick-view="backup"') && indexHtml.includes('id="quickBackupCount"'), "Round 77: zero-score current records lost their dedicated backup view");
check(!appSource.includes("tierLabel(item.tier)"), "Round 77: visible cards still render the legacy A/B/C/D tier label instead of personal-fit wording");
check(
...
check(
  auditedMain.every((item) => {
    ...
    if (language === "english") return score <= 8;
    if (language === "englishLikely") return score <= 10;
    if (language === "unknown") return score <= 12;
    if (language === "spanish") return score <= 5;
    if (language === "spanishLikely") return score <= 7;
    if (language === "foreign") return score <= 4;
    if (language === "chineseForeign") return score <= 38;
    return true;
  }),
  "外语岗位突破了个人语言门槛分数上限",
);
...
check(test.applicationLanguagePath(test.allData.find((item) => item.id === 188)).key === "englishSpanishLikely", "eseOese 明确英语加本地西语环境没有进入复合语言门槛层");
...
check(test.toLinks(test.allData.find((item) => item.id === 930823))[0]?.includes("marketing-paid-motion-designer-global-markets-316734") && !test.toLinks(test.allData.find((item) => item.id === 930823)).some((url) => /306539/i.test(url)), "Factorial Paid Motion Designer 当前官方 requisition 没有成为第一投递入口");
...
check([170, 445, 1081].every((id) => auditedMain.some((item) => Number(item.id) === id)) && test.applicationStatus(test.allData.find((item) => Number(item.id) === 1108)).key === "closed", "第三批研究库记录未保留当前机会或 Glovo 过期历史状态");
...
check(
  [930816, 930818, 930819, 93, 1296, 1293, 37, 279].every((id) => auditedMain.some((item) => Number(item.id) === id)) &&
    ...
    !auditedMain.some((item) => Number(item.id) === 930817),
  "Round 73: 第八批当前机会未完整保留，或 Eat Nudes 已关闭卡仍泄漏进主表",
);
check([930820, 930821, 930823, 930900, 228].every((id) => auditedMain.some((item) => Number(item.id) === id)) && !auditedMain.some((item) => [178, 930822].includes(Number(item.id))), "第九批当前机会、Bending Spoons 规范卡或已关闭记录状态错误");
...
check([930838, 930841, 930842, 930843, 930844].every((id) => auditedMain.some((item) => Number(item.id) === id)) && !auditedMain.some((item) => Number(item.id) === 930840), "第十三批当前岗位或 Round 76 Zurich 非设计排除状态错误");
check([930838, 930841, 930842, 930843, 930844].every((id) => test.applicationStatus(test.allData.find((item) => Number(item.id) === id)).key === "live") && test.applicationStatus(test.allData.find((item) => Number(item.id) === 930840)).key === "closed", "第十三批当前岗位或 Zurich 历史排除状态错误");
...
check(test.applicationStatus(test.allData.find((item) => Number(item.id) === 1301)).key === "live", "The Colour Monster 的近期精确招聘页未保留为 live");
...
check(indexHtml.includes('option value="englishSpanish"') && indexHtml.includes('option value="englishSpanishLikely"'), "语言筛选缺少英西复合门槛选项");
check(
...
check(
  [979, 1025, 1049, 654, 649, 930848, 214, 327, 141, 930828, 930875, 407, 330, 336, 130, 1264, 1288, 1248, 930877, 109, 117, 1023, 385, 841, 903, 855, 875, 397, 1274, 217, 308, 1238, 1262, 848, 213, 930717, 930852, 930849, 537].every((id) =>
    round33AuditItems.some((item) => Number(item.id) === id),
  ) && test.toLinks(test.allData.find((item) => Number(item.id) === 930864)).some((url) => /4432554030/i.test(url)),
...
check(/当前 211 个机会的完整、严格降序导出/.test(readmeSource), "README 当前主表导出数量没有与 211 张现行卡同步");
...
const nonActionableChineseRoutes = test.dedupedData.flatMap((item) =>
  test
    .toLinks(item)
    .filter((url) =>
      /infohuaxin\.com\/show(?:class|info)\.asp|eulam\.infohuaxin\.com\/listings|xihua\.es\/forum\.php\?.*mod=forumdisplay|bbs\.eus\/forum\.php\?(?!.*mod=viewthread)|huarenjiewang\.com\/category-|es02\.com\/?$/i.test(url) &&
      !(Number(item.id) === 930904 && /infohuaxin\.com\/showinfo\.asp\?id=4281407$/i.test(url)),
    ),
);
...
check(test.allData.find((item) => item.id === 859).postedAt === "2026-07-22", "Stripe Motion Designer 官方发布日期没有保留");
...
check(test.applicationStatus(test.allData.find((item) => item.id === 870)).key === "live", "Flummox 当前JOIN职位没有从旧404镜像中恢复");
check(test.applicationLanguagePath(test.allData.find((item) => item.id === 870)).key === "english", "Flummox 流利英语硬门槛没有正确计入");
...
check(
  test.allData.find((item) => item.id === 427).tier === "X" &&
    test.applicationStatus(test.allData.find((item) => item.id === 427)).key === "closed" &&
    test.toLinks(test.allData.find((item) => item.id === 427)).some((url) => /d47d1c9b-3e07-4878-84d7-5841dd3930d8/i.test(url)) &&
    test.toLinks(test.allData.find((item) => item.id === 427)).some((url) => /4436834624/i.test(url)),
  "Round 73: Revolut Employer Branding closure or its exact official and employer-detail evidence is missing",
);
...
check(test.applicationLanguagePath(test.allData.find((item) => item.id === 313)).key === "englishSpanish", "Mapit 母语西语与英语 B2 双硬门槛没有进入语言筛选");
...
check(test.CURATED[910].changeType === "round-76-official-ashby-live-refresh" && test.CURATED[912].changeType === "refresh", "LABHOUSE Round 76 官方页面刷新或第二十轮状态刷新丢失");
...
check(test.CURATED[913].changeType === "refresh" && test.CURATED[914].changeType === "round-74-official-greenhouse-live-refresh", "Trivelta 最新状态提升没有正确标记");
...
check(r588Thru?.tier === "X" && test.locationBucket(r588Thru) === "barcelona" && test.applicationStatus(r588Thru).key === "closed" && /NO JOB OPENINGS/i.test(r588Thru.status), "Round 66: THRU's explicit no-openings statement did not archive the stale role body");
...
check(r701Runroom && test.applicationStatus(r701Runroom).key === "closed" && r701Runroom.tier === "X" && test.locationBucket(r701Runroom) === "barcelona", "Round 54: Runroom no-longer-accepting correction regressed");
const r701DdbJunior = test.allData.find((item) => item.id === 960);
check(r701DdbJunior && test.applicationStatus(r701DdbJunior).key === "closed" && r701DdbJunior.tier === "X" && test.locationBucket(r701DdbJunior) === "barcelona", "Round 54: DDB Art Director Junior error-redirect correction regressed");
...
check(r712ColourMonster && r712ColourMonster.score === 100 && test.locationBucket(r712ColourMonster) === "barcelona" && test.applicationStatus(r712ColourMonster).key === "live" && test.toLinks(r712ColourMonster).some((url) => /4446592473/i.test(url)), "Round 66: The Colour Monster recent exact employer detail did not remain live");
...
check(
  r20Kraken &&
    test.MY_OPPORTUNITY_SET.has(930860) &&
    test.applicationStatus(r20Kraken).key === "live" &&
    test.locationBucket(r20Kraken) === "remote" &&
    /81 listed jobs|secondary eligible locations|Spain/i.test(`${r20Kraken.status} ${test.CURATED[930860]?.statusEvidence}`) &&
    test.displayedScore(r20Kraken) === 0 &&
    test.toLinks(r20Kraken).some((url) => /jobs\.ashbyhq\.com\/kraken\.com\/8ed4c65b-aaac-40d0-9d41-423683b7a1bd/i.test(url)),
  "Round 61: Kraken Spain-eligible official requisition is missing, closed, or ranked above the user's language/experience fit",
);
...
check(r22Prime && test.MY_OPPORTUNITY_SET.has(930864) && test.applicationStatus(r22Prime).key === "live" && test.toLinks(r22Prime)[0]?.includes("4448772794") && test.toLinks(r22Prime).some((url) => /4432554030/i.test(url)), "Round 65: Prime Insights replacement route was not restored while preserving the expired route history");
...
check(test.MY_OPPORTUNITY_SET.has(930864), "Round 65: current Prime Insights replacement requisition is missing from the ranked board");
...
check(r22Main.filter((item) => test.isChineseRelevant(item) && test.applicationStatus(item).key !== "closed").length === 5, "Round 72: Chinese-relevant current opportunity count must reflect the ELIM HOME closure at 5");
...
check(test.MY_OPPORTUNITY_IDS.length === 239 && new Set(test.MY_OPPORTUNITY_IDS).size === 239, "Round 79: audited ID ledger must contain exactly 239 unique opportunities after adding Bitpanda");
check(r23Main.length === 239 && r23VisibleMain.length === 238, `Round 79: complete audited history must preserve all 239 reviewed IDs while the established duplicate identity merges in the visible corpus (${r23Main.length}/${r23VisibleMain.length})`);
check(r23Main.filter((item) => test.locationBucket(item) === "barcelona").length === 176 && r23Main.filter((item) => test.locationBucket(item) === "remote").length === 63, "Round 79: audited Barcelona/remote split must be exactly 176/63 including preserved closed history");
check(
  r23Main.filter((item) => test.applicationStatus(item).key === "live").length === 205 &&
    r23Main.filter((item) => test.applicationStatus(item).key === "verify").length === 6 &&
    r23Main.filter((item) => test.applicationStatus(item).key === "closed").length === 28,
  "Round 79: 239 reviewed IDs must resolve to 205 live, 6 verify and 28 preserved closed-history records",
);
...
check(
  test.displayedScore(r23ById(930841)) === 0 &&
    test.displayedScore(r23ById(930813)) === 0 &&
    test.applicationStatus(r23ById(930813)).key === "closed",
  "Round 79: the closed Dragons junior role or the 10-15 year lead role escaped the zero-score safeguard",
);
...
for (const id of [930866, 1108, 930838, 930845, 930827, 930880, 1038, 930705, 930708, 284, 238]) {
  const item = r23ById(id);
...
for (const id of [930844, 1105, 1107, 990, 12, 2942, 1020, 443]) {
  const item = r23ById(id);
...
check(test.applicationLanguagePath(r23ById(601)).key === "unknown" && test.scoreLanguageRisk(r23ById(601)) === "englishLikely", "Round 59: SIERRA must preserve unknown formal-language evidence while receiving the probable-English score risk");
...
check(
  r41Current.length === 211 &&
    r41Current.filter((item) => test.locationBucket(item) === "barcelona").length === 149 &&
    r41Current.filter((item) => test.locationBucket(item) === "remote").length === 62 &&
    r41LanguageCounts.chineseCheck === 2 &&
    !r41LanguageCounts.basicSpanish &&
    r41LanguageCounts.english === 70 &&
    r41LanguageCounts.unknown === 58 &&
    r41LanguageCounts.englishLikely === 3 &&
    r41LanguageCounts.englishSpanishLikely === 3 &&
    r41LanguageCounts.spanishLikely === 23 &&
    r41LanguageCounts.spanish === 33 &&
    r41LanguageCounts.englishSpanish === 11 &&
    r41LanguageCounts.foreign === 8,
  `Round 77: current ledger mismatch ${JSON.stringify({ total: r41Current.length, language: r41LanguageCounts })}`,
);
...
check(
  r51ScoringRiskCounts.chineseCheck === 2 &&
    ...
    r51ScoringRiskCounts.english === 68 &&
    r51ScoringRiskCounts.englishLikely === 57 &&
    r51ScoringRiskCounts.englishSpanishLikely === 5 &&
    !r51ScoringRiskCounts.unknown &&
    r51ScoringRiskCounts.spanishLikely === 26 &&
    r51ScoringRiskCounts.spanish === 31 &&
    r51ScoringRiskCounts.englishSpanish === 11 &&
    r51ScoringRiskCounts.foreign === 9,
  `Round 77: profile-based scoring-risk ledger mismatch ${JSON.stringify(r51ScoringRiskCounts)}`,
);
...
const r51Caps = { chinese: 100, chineseCheck: 84, chineseForeign: 38, basicSpanish: 35, unknown: 12, english: 8, englishLikely: 10, englishSpanish: 3, englishSpanishLikely: 4, spanishLikely: 7, foreign: 4, spanish: 5 };
...
check(
  r42Ametller &&
    ...
    test.applicationLanguagePath(r42Ametller).key === "unknown" &&
    test.scoreLanguageRisk(r42Ametller) === "foreign" &&
...
check([778, 920, 24, 25].every((id, index) => Number(r43Sorted[index]?.id) === id), "Round 72: current Chinese/contact-first opportunities are not ordered by role fit, language risk and practical feasibility");
check(Math.max(...r43Sorted.slice(4).map((item) => test.displayedScore(item))) <= 10, "Round 72: a pure foreign-language backup exceeds the feasibility ceiling");
...
check(test.MY_OPPORTUNITY_IDS.length === 239 && new Set(test.MY_OPPORTUNITY_IDS).size === 239, "Round 79: current refresh changed the 239-seat audited ledger unexpectedly");
...
const round53Section = "2026-08-13 Round 53 Chinese-advantage and foreign-language feasibility rescore";
const round54Section = "2026-08-13 Round 54 ranked 31-60 original-page and application-path recheck";
const round55Section = "2026-08-13 Round 55 ranked 61-90 original-page and source-deduplication recheck";
const round56Section = "2026-08-13 Round 56 ranked 91-120 original-page, language-gate and duplicate recheck";
const round57Section = "2026-08-13 Round 57 highest-score unaudited original-page and profile-fit recheck";
const round58Section = "2026-08-13 Round 58 next-highest original-page, dual-language and current-source audit";
...
const round63Section = "2026-08-13 Round 63 Barcelona exact-detail discovery and user-fit scoring";
...
const round68Section = "2026-08-14 Round 68 Chinese-source rescan and current-employer restoration";
const round69Section = "2026-08-14 Round 69 current-source discovery, top-five refresh and mirror deduplication";
const round70Section = "2026-08-14 Round 70 exact-page secondary-role triage, duplicate merge and removed-route audit";
const round71Section = "2026-08-14 Round 71 official-board Spain-remote lead and stale-search rejection audit";
...
const round79Section = "2026-08-14 Round 79 anonymous-mirror triage, Restate location correction and Dragons Junior closure audit";
...
const round76TouchedIds = [910, 930840, 930911, 78, 930825, 446, 930819, 930821, 877, 601, 930929];
...
const round79TouchedIds = [890, 238, 930813, 930834, 930837, 930847, 966, 930908, 930909, 930966, 997];
...
const round57ExpectedIds = [778, 920, 24, 25, 930834, 930837, 1107, 1092, 372, 427, 930847, 890, 1300, 930813, 930863, 930832, 930711, 930817, 930899, 910, 604, 914, 352, 633, 930836, 94, 866, 884, 930820, 207];
...
check(
  round77Gte &&
    test.MY_OPPORTUNITY_SET.has(930964) &&
    test.applicationStatus(round77Gte).key === "live" &&
    test.locationBucket(round77Gte) === "remote" &&
    test.scoreLanguageRisk(round77Gte) === "englishLikely" &&
    test.directionKey(round77Gte) === "brand" &&
    test.displayedScore(round77Gte) > 0 &&
    test.displayedScore(round77Gte) <= 10 &&
    test.toLinks(round77Gte).some((url) => /work\.gtebrands\.com\/jobs\/7732621/i.test(url)),
  "Round 77: GTE official worldwide-remote brand-visual role is missing or escaped its English/Spain safeguards",
);
for (const id of [868, 136]) {
  const item = r23ById(id);
  check(
    item && test.applicationStatus(item).key === "closed" && item.tier === "X" && test.displayedScore(item) === 0,
    `Round 77: closed/no-vacancy record ${id} still leaks into the current board`,
  );
}
for (const id of [930955, 990]) {
  check(test.applicationStatus(r23ById(id)).key === "live", `Round 77: exact current application route ${id} was not promoted to live`);
}
for (const id of [778, 920, 25, 1253, 930865, 930892]) {
  check(test.applicationStatus(r23ById(id)).key === "verify", `Round 77: evidence-limited route ${id} lost verify-first status`);
}
check(
...
check(
  round78TouchedIds.every((id) => test.CURATED[id]?.latestAuditSection === round78Section),
  "Round 78: the four exact-page refreshes lost their audit provenance",
);
check(test.latestRoundSection === round79Section, "Round 79: latest-round marker did not advance");
check(
  round79TouchedIds.every((id) => test.latestRoundItems.some((item) => Number(item.id) === id)) &&
    test.latestRoundItems.length === round79TouchedIds.length,
  "Round 79: the eleven exact-page decisions are missing from the latest audit log",
);
const round79Restate = r23ById(997);
const round79DragonsJunior = r23ById(930813);
const round79Linear = r23ById(930834);
const round79LinearMirror = r23ById(930965);
const round79Bitpanda = r23ById(966);
const round79AnonymousSenior = r23ById(930966);
check(
  round79Restate &&
    test.applicationStatus(round79Restate).key === "closed" &&
    test.locationBucket(round79Restate) === "other" &&
    test.toLinks(round79Restate).some((url) => /restate\/1192a1a4-6574-4c54-afe8-356235f82dad/i.test(url)),
  "Round 79: Restate current-but-Spain-excluded role leaked into the Spain board or lost its exact route",
);
check(
  round79DragonsJunior &&
    test.MY_OPPORTUNITY_SET.has(930813) &&
    test.applicationStatus(round79DragonsJunior).key === "closed" &&
    test.displayedScore(round79DragonsJunior) === 0 &&
    !auditedMain.some((item) => Number(item.id) === 930813) &&
    test.toLinks(round79DragonsJunior).some((url) => /joinus\.dragons-group\.com\/jobs\/junior-graphic-designer/i.test(url)),
  "Round 79: explicitly closed Dragons Junior route still leaks into the current board",
);
check(
  round79Linear &&
    round79LinearMirror &&
    test.applicationStatus(round79Linear).key === "live" &&
    test.identityKey(round79Linear) === test.identityKey(round79LinearMirror) &&
    test.dedupedData.some((item) => Number(item.id) === 930834) &&
    !test.dedupedData.some((item) => Number(item.id) === 930965) &&
    test.toLinks(round79Linear).some((url) => /Linear\/5b9997fd-7507-4437-8fd7-14178c99ab5d/i.test(url)),
  "Round 79: anonymous Jobgether Production Designer mirror did not merge into the official Linear card",
);
check(
  round79Bitpanda &&
    test.MY_OPPORTUNITY_SET.has(966) &&
    test.applicationStatus(round79Bitpanda).key === "live" &&
    test.locationBucket(round79Bitpanda) === "barcelona" &&
    test.applicationLanguagePath(round79Bitpanda).key === "english" &&
    test.experienceInfo(round79Bitpanda).key === "senior" &&
    test.displayedScore(round79Bitpanda) === 0 &&
    test.toLinks(round79Bitpanda).some((url) => /bitpanda\/jobs\/4905680101/i.test(url)),
  "Round 79: Bitpanda live Barcelona role is missing or escaped its English/senior zero-score safeguard",
);
check(
  [930908, 930909].every((id) => test.applicationStatus(r23ById(id)).key === "verify") &&
    round79AnonymousSenior &&
    !test.MY_OPPORTUNITY_SET.has(930966) &&
    test.applicationStatus(round79AnonymousSenior).key === "verify" &&
    test.displayedScore(round79AnonymousSenior) === 0,
  "Round 79: anonymous Jobgether pages were promoted beyond their evidence or feasibility",
);
...
const round76Sierra = r23ById(601);
const round76BjakLead = r23ById(930929);
check(
  round76Zurich &&
    test.applicationStatus(round76Zurich).key === "closed" &&
    round76Zurich.tier === "X" &&
    test.directionKey(round76Zurich) === "other" &&
    test.displayedScore(round76Zurich) === 0 &&
    /1368922057/.test(test.toLinks(round76Zurich)[0] || "") &&
    /non-executional|非执行|不亲自执行/i.test(`${round76Zurich.status} ${test.CURATED[930840]?.statusEvidence || ""}`),
  "Round 76: Zurich current-but-non-executional strategy role was not excluded with its official evidence",
);
check(
  round76Nude &&
    test.applicationStatus(round76Nude).key === "closed" &&
    round76Nude.tier === "X" &&
    test.displayedScore(round76Nude) === 0 &&
    /4385975298/.test(test.toLinks(round76Nude)[0] || "") &&
    /no longer loads|无法加载|入口已经失效/i.test(`${round76Nude.status} ${test.CURATED[930911]?.statusEvidence || ""}`),
  "Round 76: NUDE PROJECT removed exact route was not preserved as closed history",
);
check(
  round76Sierra &&
    test.applicationStatus(round76Sierra).key === "live" &&
    test.experienceInfo(round76Sierra).key === "senior" &&
    test.displayedScore(round76Sierra) === 0 &&
    /significant track record|成熟 Art Director/i.test(`${round76Sierra.status} ${test.CURATED[601]?.statusEvidence || ""}`) &&
    test.toLinks(round76Sierra).some((url) => /sierraproductionhouse\.com\/jobs/i.test(url)),
  "Round 76: SIERRA senior Art Director gate or current official route was not corrected",
);
check(
  round76BjakLead &&
    test.applicationStatus(round76BjakLead).key === "live" &&
    test.locationBucket(round76BjakLead) === "remote" &&
    test.applicationLanguagePath(round76BjakLead).key === "english" &&
    test.experienceInfo(round76BjakLead).key === "lead" &&
    test.displayedScore(round76BjakLead) === 0 &&
    test.toLinks(round76BjakLead).some((url) => /b2946157-811b-481b-9bc4-16c2cd1de29c/i.test(url)) &&
    test.toLinks(round76BjakLead).some((url) => /4452780415/i.test(url)),
  "Round 76: BJAK LinkedIn mirror was not merged into the existing zero-score official lead card",
);
for (const id of [910, 78, 930825, 446, 930819, 930821, 877]) {
  check(
    test.applicationStatus(r23ById(id)).key === "live" && test.CURATED[id]?.latestAuditSection === round76Section,
    `Round 76: exact current-page refresh was not preserved for ${id}`,
  );
}
...
for (const id of [890, 884, 930900, 930836, 930820, 425, 930845]) {
  check(
    test.applicationStatus(r23ById(id)).key === "live" &&
      test.CURATED[id]?.latestAuditSection === (id === 930845 ? round78Section : id === 890 ? round79Section : round75Section),
    `Round 75: exact current-page refresh was not preserved for ${id}`,
  );
}
check(
  test.applicationStatus(r23ById(1300)).key === "live" && test.CURATED[1300]?.latestAuditSection === round77Section,
  "Round 77: INFiLED current employer-page refresh was not preserved",
);
check(
...
const round74RetainedIds = round74TouchedIds.filter((id) => !round79TouchedIds.includes(id));
const round74Items = test.dedupedData.filter((item) => test.CURATED[item.id]?.latestAuditSection === round74Section);
check(
  round74RetainedIds.every((id) => round74Items.some((item) => Number(item.id) === id)) &&
    round74Items.length === round74RetainedIds.length,
  "Round 74: unchanged exact-page decisions were not preserved after later audits",
);
const round74Neo = r23ById(17);
const round74DistributedCrafts = r23ById(930962);
const round74RevolutLead = r23ById(930963);
check(
  round74Neo &&
    test.MY_OPPORTUNITY_SET.has(17) &&
    test.applicationStatus(round74Neo).key === "live" &&
    test.locationBucket(round74Neo) === "remote" &&
    test.applicationLanguagePath(round74Neo).key === "foreign" &&
    test.experienceInfo(round74Neo).key === "lead" &&
    test.displayedScore(round74Neo) === 0 &&
    test.toLinks(round74Neo).some((url) => /1D7B5D402B\/apply/i.test(url)),
  "Round 74: Neo exact Workable restoration, native-Russian gate, lead gate or zero-score safeguard regressed",
);
check(
  round74DistributedCrafts &&
    !test.MY_OPPORTUNITY_SET.has(930962) &&
    test.applicationStatus(round74DistributedCrafts).key === "closed" &&
    test.displayedScore(round74DistributedCrafts) === 0 &&
    /no longer available|no job openings/i.test(`${round74DistributedCrafts.status} ${test.CURATED[930962]?.statusEvidence || ""}`),
  "Round 74: Distributed Crafts stale search result was not preserved as exact-page closed history",
);
check(
  round74RevolutLead &&
    test.MY_OPPORTUNITY_SET.has(930963) &&
    test.applicationStatus(round74RevolutLead).key === "live" &&
    test.locationBucket(round74RevolutLead) === "barcelona" &&
    test.applicationLanguagePath(round74RevolutLead).key === "unknown" &&
    test.scoreLanguageRisk(round74RevolutLead) === "englishLikely" &&
    test.experienceInfo(round74RevolutLead).key === "lead" &&
    test.displayedScore(round74RevolutLead) === 0 &&
    test.toLinks(round74RevolutLead).some((url) => /careers\/apply\/4bb06532-5d7a-452c-9b32-bd56cace5b3e/i.test(url)),
  "Round 74: Revolut Barcelona lead role, application form, evidence-versus-risk language split or 8+ year zero-score safeguard regressed",
);
for (const id of [930863, 930899, 914]) {
  check(
    test.applicationStatus(r23ById(id)).key === "live" && test.CURATED[id]?.latestAuditSection === round74Section,
    `Round 74: exact current-page refresh was not preserved for ${id}`,
  );
}
check(
  [930834, 930837].every((id) => test.isFormalRole(r23ById(id)) && !test.isFreelanceRole(r23ById(id))) &&
    test.isFreelanceRole(r23ById(930863)) && !test.isFormalRole(r23ById(930863)),
  "Round 73: application advice text is still polluting the evidence-based full-time/freelance badge",
);
const round71Items = test.dedupedData.filter((item) => test.CURATED[item.id]?.latestAuditSection === round71Section);
check(
  [930959, 930960, 930961].every((id) =>
    round71Items.some((item) => Number(item.id) === id),
  ) && round71Items.length === 3,
  "Round 71: the three official-board decisions were not preserved after Round 72",
);
...
check(
  [870, 930949, 930950, 930951].every((id) =>
    round68Items.some((item) => Number(item.id) === id),
  ) && round68Items.length === 4,
  "Round 68: the four current-source decisions were not preserved after Round 69",
);
const round67Items = test.dedupedData.filter((item) => test.CURATED[item.id]?.latestAuditSection === round67Section);
check(
  [859, 920003, 930839, 930948].every((id) =>
    round67Items.some((item) => Number(item.id) === id),
  ) && round67Items.length === 4 && test.CURATED[930840]?.latestAuditSection === round76Section,
  "Round 67/76: the four untouched current-ATS decisions or Zurich superseding audit were not preserved",
);
const round66Items = test.dedupedData.filter(
  (item) => test.CURATED[item.id]?.latestAuditSection === round66Section,
);
check(
  [1107, 1237, 1243, 1301].every((id) =>
    round66Items.some((item) => Number(item.id) === id),
  ) && round66Items.length === 4,
  "Round 66: the four untouched exact-source decisions are missing after Round 77",
);
...
check(
  round67Zurich &&
    test.applicationStatus(round67Zurich).key === "closed" &&
    test.locationBucket(round67Zurich) === "barcelona" &&
    test.applicationLanguagePath(round67Zurich).key === "english" &&
    test.displayedScore(round67Zurich) === 0 &&
    round67Zurich.tier === "X" &&
    test.directionKey(round67Zurich) === "other" &&
    test.toLinks(round67Zurich).some((url) => /1368922057/i.test(url)),
  "Round 76: Zurich's open official page was not retained as an excluded non-design history record",
);
check(
  round67FystHistory &&
...
const round68Flummox = r23ById(870);
const round68MuskyHistory = r23ById(930949);
const round68MiinHistory = r23ById(930950);
const round68EezeHistory = r23ById(930951);
check(
  round68Flummox &&
    test.MY_OPPORTUNITY_SET.has(870) &&
    test.applicationStatus(round68Flummox).key === "live" &&
    test.locationBucket(round68Flummox) === "remote" &&
    test.applicationLanguagePath(round68Flummox).key === "english" &&
    test.displayedScore(round68Flummox) === 2.8 &&
    /16464525/i.test(test.toLinks(round68Flummox)[0] || "") &&
    /Remote \(Spain\)|minimum of 20 hours/i.test(round68Flummox.status),
  "Round 68: Flummox was not restored from the current employer requisition with its English/part-time penalty",
);
check(
  round68MuskyHistory &&
    !test.MY_OPPORTUNITY_SET.has(930949) &&
    test.applicationStatus(round68MuskyHistory).key === "closed" &&
    round68MuskyHistory.tier === "X" &&
    /expired_jd_redirect/i.test(round68MuskyHistory.status) &&
    test.toLinks(round68MuskyHistory).some((url) => /4409977351/i.test(url)),
  "Round 68: expired Musky internship search residue leaked into the current board",
);
check(
  round68MiinHistory &&
    !test.MY_OPPORTUNITY_SET.has(930950) &&
    test.applicationStatus(round68MiinHistory).key === "closed" &&
    round68MiinHistory.tier === "X" &&
    /unavailable|不可用/i.test(`${round68MiinHistory.status} ${test.CURATED[930950]?.statusEvidence || ""}`) &&
    test.toLinks(round68MiinHistory).some((url) => /f7f67953-9839-4bfb-b1a2-031cba7861b2/i.test(url)),
  "Round 68: MiiN's new-looking but closed employer URL leaked into the current board",
);
check(
  round68EezeHistory &&
    !test.MY_OPPORTUNITY_SET.has(930951) &&
    test.applicationStatus(round68EezeHistory).key === "closed" &&
    round68EezeHistory.tier === "X" &&
    test.locationBucket(round68EezeHistory) === "other" &&
    /Hammersmith/i.test(round68EezeHistory.status) &&
    test.toLinks(round68EezeHistory).some((url) => /6093226-3d-animator/i.test(url)),
  "Round 68: Eeze's reused UK 3D Animator requisition leaked into the Spain opportunity board",
);
const round69Amazon = r23ById(930952);
check(
  round69Amazon &&
    test.MY_OPPORTUNITY_SET.has(930952) &&
    test.applicationStatus(round69Amazon).key === "live" &&
    test.locationBucket(round69Amazon) === "barcelona" &&
    test.applicationLanguagePath(round69Amazon).key === "englishLikely" &&
    test.experienceInfo(round69Amazon).key === "lead" &&
    test.displayedScore(round69Amazon) === 0 &&
    test.toLinks(round69Amazon).some((url) => /4454320948/i.test(url)) &&
...
for (const [id, statusKey, score] of [[778, "verify", 81], [920, "verify", 52], [24, "live", 35.3], [25, "verify", 19.9]]) {
  ...
  check(
    item &&
      test.CURATED[id]?.latestAuditSection === round77Section &&
      test.applicationStatus(item).key === statusKey &&
      test.displayedScore(item) === score &&
      test.toLinks(item).length > 0,
    `Round 77: current top-priority page refresh or profile score drifted for ${id}`,
  );
}
check(
  test.CURATED[1300]?.latestAuditSection === round77Section &&
    test.applicationStatus(r23ById(1300)).key === "live" &&
    test.displayedScore(r23ById(1300)) === 8 &&
    test.toLinks(r23ById(1300)).length > 0,
  "Round 77: INFiLED current employer-page refresh or profile score drifted",
);
check(
  test.CURATED[930904]?.latestAuditSection === round72Section &&
    test.applicationStatus(r23ById(930904)).key === "closed" &&
    r23ById(930904).tier === "X" &&
    test.displayedScore(r23ById(930904)) === 0 &&
    /Ya no se aceptan solicitudes|不再接受申请/i.test(test.CURATED[930904]?.statusEvidence || ""),
  `Round 72: ELIM HOME exact employer closure was not preserved ${JSON.stringify({ section: test.CURATED[930904]?.latestAuditSection, status: test.applicationStatus(r23ById(930904)).key, tier: r23ById(930904)?.tier, score: test.displayedScore(r23ById(930904)), evidence: test.CURATED[930904]?.statusEvidence })}`,
);
const round69BendingSpoons = r23ById(930900);
check(
  round69BendingSpoons &&
    test.applicationStatus(round69BendingSpoons).key === "live" &&
    test.displayedScore(round69BendingSpoons) === 8 &&
    test.toLinks(round69BendingSpoons).some((url) => /69d588c142e4c7b7a3dd14ee/i.test(url)) &&
    test.toLinks(round69BendingSpoons).some((url) => /4453273680/i.test(url)) &&
    test.toLinks(round69BendingSpoons).some((url) => /4453267708/i.test(url)),
  "Round 69: Bending Spoons location mirrors were not consolidated into the current canonical role",
);
const round69MiinHead = r23ById(109);
check(
  round69MiinHead &&
    !test.MY_OPPORTUNITY_SET.has(109) &&
    test.applicationStatus(round69MiinHead).key === "closed" &&
    !r41Current.some((item) => Number(item.id) === 109) &&
    test.toLinks(round69MiinHead).some((url) => /empleos\.miin-cosmetics\.com\/jobs\/head-of-creative-and-content-barcelona-40h/i.test(url)) &&
    /Closed job listing/i.test(test.CURATED[109]?.statusEvidence || ""),
  "Round 69: MiiN Head of Creative and Content was not moved to preserved history using the exact employer closure",
);
const round70AnonymousFashion = r23ById(930955);
check(
  round70AnonymousFashion &&
    test.MY_OPPORTUNITY_SET.has(930955) &&
    test.applicationStatus(round70AnonymousFashion).key === "live" &&
    test.locationBucket(round70AnonymousFashion) === "barcelona" &&
    test.applicationLanguagePath(round70AnonymousFashion).key === "english" &&
    test.experienceInfo(round70AnonymousFashion).key === "mid" &&
    test.hasOpaqueEmployerRisk(round70AnonymousFashion) &&
    test.displayedScore(round70AnonymousFashion) <= 8 &&
    test.toLinks(round70AnonymousFashion).some((url) => /4446631402/i.test(url)),
  "Round 77: anonymous Barcelona fashion web-design role lost its live page, English, experience or employer-risk safeguards",
);
const round70Fitness = r23ById(930956);
check(
  round70Fitness &&
    test.MY_OPPORTUNITY_SET.has(930956) &&
    test.applicationStatus(round70Fitness).key === "live" &&
    test.locationBucket(round70Fitness) === "barcelona" &&
    test.applicationLanguagePath(round70Fitness).key === "englishSpanish" &&
    test.displayedScore(round70Fitness) <= 3 &&
    /permanent|indefinido/i.test(round70Fitness.status) &&
    /travel|出差/i.test(`${round70Fitness.status} ${test.CURATED[930956]?.reason || ""}`) &&
    test.toLinks(round70Fitness).some((url) => /4452457993/i.test(url)),
  `Round 70: Fitness Park filmmaker lost its current Barcelona route, permanent terms, travel warning or double-language ceiling ${JSON.stringify({ status: test.applicationStatus(round70Fitness).key, location: test.locationBucket(round70Fitness), language: test.applicationLanguagePath(round70Fitness).key, score: test.displayedScore(round70Fitness), links: test.toLinks(round70Fitness), statusText: round70Fitness.status })}`,
);
const round70Cosmo = r23ById(930957);
check(
  round70Cosmo &&
    test.MY_OPPORTUNITY_SET.has(930957) &&
    test.applicationStatus(round70Cosmo).key === "live" &&
    test.locationBucket(round70Cosmo) === "barcelona" &&
    test.applicationLanguagePath(round70Cosmo).key === "englishSpanish" &&
    test.experienceInfo(round70Cosmo).key === "mid" &&
    test.displayedScore(round70Cosmo) <= 3 &&
    /fragrance-industry experience|香氛行业经验/i.test(`${round70Cosmo.status} ${test.CURATED[930957]?.experienceLabel || ""}`) &&
    test.toLinks(round70Cosmo).some((url) => /4451577111/i.test(url)),
  "Round 70: Cosmo social/brand-content role lost its exact route, fragrance gate or explicit English-and-Spanish cap",
);
for (const [id, route, evidence] of [
  [930953, /4450790723/i, /A10496045|production management|制片运营/i],
  [930954, /4426358914/i, /not an executional role|不是 executional role/i],
  [930958, /4DC9922BE5/i, /generic Mindrift - Current Openings|总职位板/i],
]) {
  const item = r23ById(id);
  check(
    item &&
      ...
      test.applicationStatus(item).key === "closed" &&
      item.tier === "X" &&
      test.toLinks(item).some((url) => route.test(url)) &&
      evidence.test(`${item.status || ""} ${test.CURATED[id]?.statusEvidence || ""}`),
    `Round 70: research-only or removed-route record ${id} leaked into the current opportunity board or lost its exact evidence`,
  );
}
check(
  test.CURATED[930719]?.latestAuditSection === round78Section &&
    ...
    !test.toLinks(r23ById(930719)).some((url) => /7993316-digital-graphic-designer/i.test(url)),
  "Round 70: JobsThatFit mirror was not merged into the Barcelona Adsmurai canonical card or reintroduced the Madrid requisition",
);
check(
  test.CURATED[352]?.latestAuditSection === round73Section &&
    /byblankspace\.com\/careers\/junior-3d-designer/i.test(test.toLinks(r23ById(352))[0] || "") &&
    test.toLinks(r23ById(352)).some((url) => /byblankspace\.com\/careers\/junior-3d-designer/i.test(url)) &&
    test.toLinks(r23ById(352)).some((url) => /4447843181/i.test(url)),
  "Round 73: Blank Studio live refresh lost its employer-careers canonical route or LinkedIn mirror",
);
const round71Bjak = r23ById(930959);
check(
  round71Bjak &&
    test.MY_OPPORTUNITY_SET.has(930959) &&
    test.applicationStatus(round71Bjak).key === "live" &&
    test.locationBucket(round71Bjak) === "remote" &&
    test.applicationLanguagePath(round71Bjak).key === "english" &&
    test.experienceInfo(round71Bjak).key === "lead" &&
    test.directionKey(round71Bjak) === "brand" &&
    test.displayedScore(round71Bjak) === 0 &&
    test.toLinks(round71Bjak).some((url) => /6a2445a5-6d17-4362-b35e-05033634b39d/i.test(url)) &&
    /isListed=true|Spain.*FullTime|Spain、FullTime/i.test(`${round71Bjak.status || ""} ${test.CURATED[930959]?.statusEvidence || ""}`),
  "Round 71: BJAK Spain-remote brand lead lost its official route, English/lead barriers or zero-score safeguard",
);
for (const [id, route, evidence] of [
  [930960, /doist\.com\/careers/i, /No open roles/i],
  [930961, /speexx\.com\/all-jobs/i, /没有 Brand Production Designer|no Brand Production Designer/i],
]) {
  const item = r23ById(id);
  check(
    item &&
      ...
      test.toLinks(item).some((url) => route.test(url)) &&
      evidence.test(`${item.status || ""} ${test.CURATED[id]?.statusEvidence || ""}`),
    `Round 71: stale official-board search result ${id} leaked into the current board or lost its rejection evidence`,
  );
}
const round66VerifyIds = [25, 778, 920, 1253, 930865, 930892];
check(
  round66VerifyIds.every((id) => {
    const item = r23ById(id);
    return item && test.applicationStatus(item).key === "verify" && test.toLinks(item).length > 0;
  }),
  "Round 77: an evidence-limited route escaped the six-record verify-first queue or lost its source",
);
const round66Ricaris = r23ById(1243);
check(
  round66Ricaris &&
    test.applicationStatus(round66Ricaris).key === "live" &&
    test.locationBucket(round66Ricaris) === "remote" &&
    test.experienceInfo(round66Ricaris).key === "senior" &&
    test.displayedScore(round66Ricaris) === 0 &&
    /active=true/i.test(`${round66Ricaris.status} ${test.CURATED[1243]?.statusEvidence || ""}`) &&
    test.toLinks(round66Ricaris).some((url) => /app\.dover\.com\/apply\/ricarisjoinus\/b1bcd63b-91fd-4a6d-b71b-ac6285087d10/i.test(url)),
  "Round 66: Ricaris did not retain its active official application, remote correction, senior gate or score floor",
);
const round66ColourMonster = r23ById(1301);
check(
  round66ColourMonster &&
    test.applicationStatus(round66ColourMonster).key === "live" &&
    test.locationBucket(round66ColourMonster) === "barcelona" &&
    test.applicationLanguagePath(round66ColourMonster).key === "foreign" &&
    test.displayedScore(round66ColourMonster) === 4 &&
    test.toLinks(round66ColourMonster).some((url) => /4446592473/i.test(url)),
  "Round 66: The Colour Monster current vacancy lost its exact route or compound-language score cap",
);
check(
  [1107, 1237].every((id) => {
    const item = r23ById(id);
    return item && test.applicationStatus(item).key === "closed" && item.tier === "X" && !r41Current.some((current) => Number(current.id) === id) && test.toLinks(item).length > 0;
  }),
  "Round 66: a source-proven closed record was deleted from history or leaked into the current board",
);
check(
  [930905, 930906].every((id) => test.CURATED[id]?.latestAuditSection === round62Section) &&
    [930908, 930909].every((id) => test.CURATED[id]?.latestAuditSection === round79Section) &&
    test.CURATED[930907]?.latestAuditSection === round65Section,
  "Round 62/65: prior Jobgether decisions or the new duplicate-merge provenance was lost",
);
check(
...
for (const id of [930907, 930908, 930909]) {
  ...
  check(
    item &&
      ...
      test.toLinks(item).length === (id === 930907 ? 2 : 1),
...
check(
  [930860, 55, 1234, 1007].every((id) => test.CURATED[id]?.latestAuditSection === round61Section) &&
    test.CURATED[930904]?.latestAuditSection === round72Section &&
    [153, 396].every((id) => test.CURATED[id]?.latestAuditSection === round63Section),
  "Round 61/63/72: scoring/source provenance was lost after the current audit",
);
const round64CurrentIds = [930917, 930918, 930919, 930920];
check(
  round64CurrentIds.every((id) => {
    const item = r23ById(id);
    return item && test.MY_OPPORTUNITY_SET.has(id) && test.applicationStatus(item).key === "live" && test.locationBucket(item) === "barcelona" && test.toLinks(item).length > 0;
  }),
  "Round 64: one of the four independent Barcelona secondary opportunities is missing, closed or lacks an exact route",
);
check(
  test.applicationLanguagePath(r23ById(930917)).key === "spanishLikely" &&
    test.experienceInfo(r23ById(930917)).key === "open" &&
    test.displayedScore(r23ById(930917)) === 5 &&
    /4450472949/.test(test.toLinks(r23ById(930917))[0] || "") &&
    test.applicationLanguagePath(r23ById(930918)).key === "spanishLikely" &&
    test.isInternshipRole(r23ById(930918)) &&
    test.displayedScore(r23ById(930918)) === 0 &&
    /4452752640/.test(test.toLinks(r23ById(930918))[0] || "") &&
    test.applicationLanguagePath(r23ById(930919)).key === "englishSpanish" &&
    test.experienceInfo(r23ById(930919)).key === "junior" &&
    test.displayedScore(r23ById(930919)) === 2.2 &&
    /4450097645/.test(test.toLinks(r23ById(930919))[0] || "") &&
    test.applicationLanguagePath(r23ById(930920)).key === "foreign" &&
    test.isInternshipRole(r23ById(930920)) &&
    test.displayedScore(r23ById(930920)) === 0 &&
    /4453697814/.test(test.toLinks(r23ById(930920))[0] || ""),
  "Round 64: a new secondary role lost its language barrier, seniority, low score or exact detail evidence",
);
check(
  [930921, 930922, 930923].every((id) => {
    const item = r23ById(id);
    return item && !test.MY_OPPORTUNITY_SET.has(id) && test.applicationStatus(item).key === "closed" && item.tier === "X" && test.toLinks(item).length === 1;
  }) &&
    [930924, 930925, 930926, 930927, 930928].every((id) => {
      const item = r23ById(id);
      return item && !test.MY_OPPORTUNITY_SET.has(id) && test.applicationStatus(item).key === "closed" && item.tier === "X" && test.toLinks(item).length === 1;
    }),
  "Round 64: duplicate mirrors or inspected non-design false positives leaked into the active board",
);
check(
  test.applicationStatus(r23ById(930843)).key === "live" &&
    test.displayedScore(r23ById(930843)) === 0 &&
    test.toLinks(r23ById(930843)).some((url) => /talentclue\.com\/node\/127743936\/66909823/i.test(url)) &&
    test.toLinks(r23ById(930843)).some((url) => /4453114943/i.test(url)) &&
    test.applicationStatus(r23ById(930866)).key === "live" &&
    test.displayedScore(r23ById(930866)) === 5.8 &&
    test.toLinks(r23ById(930866)).some((url) => /jobs\.ashbyhq\.com\/preply\/3b86049e/i.test(url)) &&
    test.toLinks(r23ById(930866)).some((url) => /4442687317/i.test(url)),
  "Round 64: Equipo Singular or Preply current duplicate route was not merged into the stronger canonical card",
);
const round65CurrentIds = [930864, 930929, 930930, 930931, 930932, 930933];
check(
  round65CurrentIds.every((id) => {
    const item = r23ById(id);
    return item && test.MY_OPPORTUNITY_SET.has(id) && test.applicationStatus(item).key === "live" && test.locationBucket(item) === "remote" && test.toLinks(item).length > 0;
  }),
  "Round 65: one of the six restored or new remote opportunities is missing, closed, outside the remote board or lacks an exact route",
);
check(
  test.applicationLanguagePath(r23ById(930864)).key === "unknown" &&
    test.scoreLanguageRisk(r23ById(930864)) === "englishLikely" &&
    test.experienceInfo(r23ById(930864)).key === "unknown" &&
    test.displayedScore(r23ById(930864)) === 4.8 &&
    /4448772794/.test(test.toLinks(r23ById(930864))[0] || "") &&
    test.toLinks(r23ById(930864)).some((url) => /4432554030/.test(url)),
  "Round 65: Prime Insights Graphic Designer lost its replacement route, expired-route history, probable-English risk or corrected score",
);
check(
  test.applicationLanguagePath(r23ById(930933)).key === "unknown" &&
    test.scoreLanguageRisk(r23ById(930933)) === "englishLikely" &&
    test.experienceInfo(r23ById(930933)).key === "junior" &&
    test.displayedScore(r23ById(930933)) === 10 &&
    /4453673807/.test(test.toLinks(r23ById(930933))[0] || ""),
  "Round 65: Forwwward Europe-remote junior visual role lost its language cap, junior gate, score or exact employer route",
);
check(
  test.applicationLanguagePath(r23ById(930932)).key === "unknown" &&
    test.scoreLanguageRisk(r23ById(930932)) === "englishLikely" &&
    test.experienceInfo(r23ById(930932)).key === "unknown" &&
    test.displayedScore(r23ById(930932)) === 2 &&
    /4448780370/.test(test.toLinks(r23ById(930932))[0] || ""),
  "Round 65: Prime Insights Motion Designer lost its labor conflict, probable-English risk, score or exact employer route",
);
check(
  [930929, 930930, 930931].every((id) => {
    const item = r23ById(id);
    return item && test.applicationLanguagePath(item).key === "english" && ["lead", "senior"].includes(test.experienceInfo(item).key) && test.displayedScore(item) === 0 && /jobs\.ashbyhq\.com\/bjakcareer/i.test(test.toLinks(item)[0] || "");
  }),
  "Round 65: a BJAK Spain-remote role escaped its explicit-English and lead/senior feasibility penalties or lost the official Ashby route",
);
check(
  [930934, 930935, 930936, 930937, 930938, 930939, 930940, 930941, 930942, 930943, 930944, 930945, 930946, 930947].every((id) => {
    const item = r23ById(id);
    return item && !test.MY_OPPORTUNITY_SET.has(id) && test.applicationStatus(item).key === "closed" && item.tier === "X" && test.toLinks(item).length > 0;
  }),
  "Round 65: a duplicate, non-design, volunteer or out-of-area false positive leaked into the current opportunity board",
);
const round63Mylva = r23ById(930910);
const round63Nude = r23ById(930911);
const round63TenthDigital = r23ById(930913);
const round63TenthBlender = r23ById(930914);
const round63Rv = r23ById(396);
check(
  [round63Mylva, round63TenthDigital, round63TenthBlender].every(
    (item) =>
      item &&
      test.MY_OPPORTUNITY_SET.has(Number(item.id)) &&
      test.applicationStatus(item).key === "live" &&
      test.locationBucket(item) === "barcelona" &&
      test.toLinks(item).length > 0,
  ) && test.applicationStatus(round63Nude).key === "closed",
  "Round 63/76: one of the three untouched current Barcelona roles is missing or NUDE closure was lost",
);
check(
  test.applicationLanguagePath(round63Mylva).key === "spanishLikely" &&
    test.experienceInfo(round63Mylva).key === "senior" &&
    test.displayedScore(round63Mylva) === 0 &&
    test.toLinks(round63Mylva).some((url) => /PwVnx9/i.test(url)),
  "Round 63: Mylva lost its exact page, seniority gate, likely-Spanish risk or zero personal-fit score",
);
check(
  test.applicationStatus(round63Nude).key === "closed" &&
    test.applicationLanguagePath(round63Nude).key === "spanishLikely" &&
    test.experienceInfo(round63Nude).key === "junior" &&
    test.displayedScore(round63Nude) === 0 &&
    round63Nude.tier === "X" &&
    /4385975298/.test(test.toLinks(round63Nude)[0] || ""),
  "Round 76: NUDE PROJECT lost its closure, junior history, likely-Spanish or exact-page evidence",
);
for (const item of [round63TenthDigital, round63TenthBlender]) {
  check(
    test.applicationLanguagePath(item).key === "english" &&
      test.scoreLanguageRisk(item) === "englishSpanishLikely" &&
      test.experienceInfo(item).key === "junior" &&
      test.displayedScore(item) === 3.6 &&
      test.toLinks(item).some((url) => /^mailto:a\.soffia@tenthrevolution\.com$/i.test(url)),
    `Round 63: Tenth Revolution ${item?.id} lost the English B2, local-language, freelance or contact safeguards`,
  );
}
check(
  round63Rv &&
    test.MY_OPPORTUNITY_SET.has(396) &&
    test.applicationStatus(round63Rv).key === "live" &&
    test.applicationLanguagePath(round63Rv).key === "englishSpanish" &&
    test.displayedScore(round63Rv) === 2.6 &&
    test.toLinks(round63Rv).some((url) => /DLOPKX/i.test(url)) &&
    test.toLinks(round63Rv).some((url) => /9wP83M/i.test(url)),
  "Round 63: RV Group duplicate source routes were not merged into the existing low-score canonical card",
);
check(
  [930915, 930916, 153].every((id) => {
    const item = r23ById(id);
    return item && test.applicationStatus(item).key === "closed" && (id === 153 || !test.MY_OPPORTUNITY_SET.has(id));
...
check(round59ExpectedIds.filter((id) => ![859, 601].includes(id)).every((id) => test.CURATED[id]?.latestAuditSection === round59Section) && test.CURATED[859]?.latestAuditSection === round67Section && test.CURATED[601]?.latestAuditSection === round76Section, "Round 59/67/76: provenance was lost after the current Stripe or SIERRA refresh");
const round60Elimhome = r23ById(930904);
const round60ElimhomeLinks = test.toLinks(round60Elimhome);
check(
  round60Elimhome &&
    test.MY_OPPORTUNITY_SET.has(930904) &&
    test.applicationStatus(round60Elimhome).key === "closed" &&
    round60Elimhome.tier === "X" &&
    test.locationBucket(round60Elimhome) === "barcelona" &&
    test.applicationLanguagePath(round60Elimhome).key === "chineseCheck" &&
    test.scoreLanguageRisk(round60Elimhome) === "chineseForeign" &&
    test.directionKey(round60Elimhome) === "other" &&
    test.displayedScore(round60Elimhome) === 0,
  `Round 72: ELIM HOME must retain Chinese-contact evidence while staying in closed history ${JSON.stringify({ status: test.applicationStatus(round60Elimhome).key, tier: round60Elimhome?.tier, location: test.locationBucket(round60Elimhome), language: test.applicationLanguagePath(round60Elimhome).key, risk: test.scoreLanguageRisk(round60Elimhome), direction: test.directionKey(round60Elimhome), score: test.displayedScore(round60Elimhome) })}`,
);
check(
  round60ElimhomeLinks.some((url) => /linkedin\.com\/jobs\/view\/.*4450494402/i.test(url)) &&
    round60ElimhomeLinks.some((url) => /infohuaxin\.com\/showinfo\.asp\?id=4281407$/i.test(url)) &&
    round60ElimhomeLinks.some((url) => /^tel:\+34656656158$/i.test(url)) &&
    !round60ElimhomeLinks.some((url) => /infohuaxin\.com\/showclass\.asp/i.test(url)),
  "Round 60: ELIMHOME exact detail and phone route are missing, or the generic category page leaked into card actions",
);
check(
  test.displayedScore(r23ById(778)) > test.displayedScore(r23ById(920)) &&
    test.displayedScore(r23ById(920)) > test.displayedScore(r23ById(24)) &&
    test.displayedScore(r23ById(24)) > test.displayedScore(r23ById(25)) &&
    test.displayedScore(r23ById(25)) > test.displayedScore(r23ById(1300)),
  "Round 72: the current Chinese/contact shortlist no longer follows the user's role-fit and language-feasibility scoring",
);
check(
  test.applicationLanguagePath(r23ById(55)).key === "unknown" &&
    test.scoreLanguageRisk(r23ById(55)) === "englishLikely" &&
    test.CURATED[55]?.latestAuditSection === round61Section,
  "Round 61: BCome inherited a sales-role language clause or lost the probable-English scoring risk",
);
for (const [id, evidence] of [[1007, /410 Gone|HTTP 410/i], [153, /R00000004175039|zero matching jobs/i], [1234, /no LearnWise design vacancy|停止接收状态/i]]) {
  const item = r23ById(id);
  check(
    item && test.applicationStatus(item).key === "closed" && item.tier === "X" && evidence.test(`${item.status || ""} ${test.CURATED[id]?.statusEvidence || ""}`),
...
check(test.applicationLanguagePath(r23ById(352)).key === "unknown" && test.scoreLanguageRisk(r23ById(352)) === "englishLikely" && test.displayedScore(r23ById(352)) === 10, "Round 73: Blank Studio lost its evidence-unknown / English-likely distinction or brand-direction ceiling");
check(test.applicationStatus(r23ById(930817)).key === "closed" && test.displayedScore(r23ById(930817)) === 0, "Round 73: Eat Nudes exact closed state or zero-score history safeguard regressed");
check(test.applicationLanguagePath(r23ById(930711)).key === "unknown" && test.scoreLanguageRisk(r23ById(930711)) === "spanishLikely" && test.displayedScore(r23ById(930711)) === 5, "Round 73: JUNGLE/MeMe retained the false bilingual-English claim or escaped the social-direction ceiling");
check(test.applicationLanguagePath(r23ById(633)).key === "spanish" && test.displayedScore(r23ById(633)) === 5, "Round 57: Puig explicit fluent English-and-Spanish gate was not applied");
for (const [id, expectedScore] of [[930832, 6.1], [604, 4.7], [94, 4.9], [866, 4.7]]) {
  check(test.experienceInfo(r23ById(id)).key === "unknown" && test.displayedScore(r23ById(id)) === expectedScore, `Round 57: ${id} retained invented fixed-year mid-level experience or the corrected score drifted`);
}
...
check(round58ExpectedMainIds.filter((id) => ![55, 396, 930840, 930866].includes(id) && !round73TouchedSet.has(id) && !round75TouchedSet.has(id) && !round76TouchedSet.has(id)).every((id) => test.CURATED[id]?.latestAuditSection === round58Section) && test.CURATED[930840]?.latestAuditSection === round76Section, "Round 58/73/75/76: one or more unchanged next-highest cards lost provenance or Zurich's exclusion refresh");
check(round59ExpectedIds.filter((id) => ![859, 601].includes(id)).every((id) => test.CURATED[id]?.latestAuditSection === round59Section) && test.CURATED[859]?.latestAuditSection === round67Section && test.CURATED[601]?.latestAuditSection === round76Section, "Round 59/67/76: one or more remaining cards lost provenance after Stripe or SIERRA refresh");
...
check(test.applicationLanguagePath(r23ById(188)).key === "englishSpanishLikely" && test.displayedScore(r23ById(188)) <= 4, "Round 59: eseOese lost its explicit-English plus probable-Spanish gate");
for (const id of [162, 296, 601]) {
  check(test.applicationLanguagePath(r23ById(id)).key === "unknown" && test.scoreLanguageRisk(r23ById(id)) === "englishLikely", `Round 59: ${id} no longer separates unstated formal language from probable-English scoring risk`);
}
check(test.applicationLanguagePath(r23ById(864)).key === "unknown" && test.scoreLanguageRisk(r23ById(864)) === "foreign" && test.applicationStatus(r23ById(864)).key === "live", "Round 59: Ametller Catalan operating risk or current Workday status regressed");
check(test.experienceInfo(r23ById(601)).key === "senior" && test.displayedScore(r23ById(601)) === 0 && /significant track record|成熟 Art Director/i.test(`${r23ById(601)?.status || ""} ${test.CURATED[601]?.statusEvidence || ""}`), "Round 76: SIERRA senior Art Director gate or zero-score correction regressed");
check(r23ById(930844).postedAt === "2026-03-26" && r23ById(1029).postedAt === "2025-09-24", "Round 59: official Synthesia or Together publication date correction regressed");
check(test.applicationLanguagePath(r23ById(930874)).key === "english" && test.displayedScore(r23ById(930874)) === 6.9, "Round 73: Ogilvy lost the explicit application-form English gate or motion-direction ceiling");
check(test.applicationLanguagePath(r23ById(930882)).key === "englishLikely" && /Spain.+资格需确认/i.test(test.locationLabel(r23ById(930882))) && test.displayedScore(r23ById(930882)) === 4.2, "Round 58: Duna still overstates Spain remote eligibility or lost its probable-English penalty");
check(test.applicationLanguagePath(r23ById(930823)).key === "english" && test.experienceInfo(r23ById(930823)).key === "unknown" && test.displayedScore(r23ById(930823)) === 4.2, "Round 58: Factorial retained an invented fixed-year experience band or lost the explicit English gate");
check(test.experienceInfo(r23ById(930866)).key === "junior" && test.displayedScore(r23ById(930866)) === 5.8, "Round 73: Preply explicit 2+ years or social-direction English ceiling regressed");
check(test.experienceInfo(r23ById(1038)).key === "unknown" && test.displayedScore(r23ById(1038)) === 2.3, "Round 58: Lodgify Motion Designer retained an invented fixed-year experience band");
for (const id of [396, 856, 930868, 930831]) {
  check(test.applicationLanguagePath(r23ById(id)).key === "englishSpanish" && test.displayedScore(r23ById(id)) <= 3, `Round 58: ${id} lost its explicit English-and-Spanish compound gate`);
}
for (const id of [930816, 930833]) {
  check(test.applicationLanguagePath(r23ById(id)).key === "englishSpanishLikely" && test.displayedScore(r23ById(id)) <= 4, `Round 58: ${id} lost its explicit-English plus probable-Spanish gate`);
}
for (const id of [930819, 930821]) {
  check(test.applicationLanguagePath(r23ById(id)).key === "spanishLikely" && test.displayedScore(r23ById(id)) === 6.7, `Round 73: ${id} presents an unstated Spanish level as a hard requirement or escaped the production-direction ceiling`);
}
const round58Ethos = r23ById(930903);
check(round58Ethos && !test.MY_OPPORTUNITY_SET.has(930903) && test.applicationStatus(round58Ethos).key === "live" && test.locationBucket(round58Ethos) === "remote" && test.scoreLanguageRisk(round58Ethos) === "englishLikely" && test.experienceInfo(round58Ethos).key === "senior" && test.displayedScore(round58Ethos) === 0 && test.toLinks(round58Ethos).length === 2, "Round 58: duplicate Ethos expert-project discovery was not consolidated as a zero-score research-library backup");
check([105, 960, 1108].every((id) => test.applicationStatus(r23ById(id)).key === "closed" && !r41Current.some((item) => Number(item.id) === id)), "Round 54: a directly closed application route still leaks into the current board");
check(test.applicationStatus(r23ById(886)).key === "live" && test.toLinks(r23ById(886))[0]?.includes("social-art-director-edicion-de-imagen-y-video-meme-barcelona-314341"), "Round 54: JUNGLE current renamed requisition was not repaired to the exact official route");
const round54BendingSpoons = r23ById(930900);
const round54EquipoSingular = r23ById(930901);
const round55EquipoSingularCanonical = r23ById(930843);
const round54OgilvySenior = r23ById(930902);
check(round54BendingSpoons && test.applicationStatus(round54BendingSpoons).key === "live" && test.locationBucket(round54BendingSpoons) === "remote" && test.applicationLanguagePath(round54BendingSpoons).key === "english" && test.experienceInfo(round54BendingSpoons).key === "open" && test.displayedScore(round54BendingSpoons) === 8 && test.toLinks(round54BendingSpoons).some((url) => /69d588c142e4c7b7a3dd14ee/i.test(url)), "Round 54: Bending Spoons Media Designer lost its official route, English gate, open seniority or score cap");
const round56BendingDuplicate = r23ById(178);
check(round56BendingDuplicate && test.applicationStatus(round56BendingDuplicate).key === "closed" && !r41Current.some((item) => Number(item.id) === 178) && test.identityKey(round56BendingDuplicate) === test.identityKey(round54BendingSpoons) && test.dedupedData.filter((item) => test.identityKey(item) === test.identityKey(round54BendingSpoons)).length === 1, "Round 56: duplicate Bending Spoons Media Designer still counts as an independent current vacancy");
for (const id of [1002, 985, 1021, 990, 37, 12, 930865, 930812, 1287]) {
  const item = r23ById(id);
  check(item && test.applicationLanguagePath(item).key === "unknown" && test.scoreLanguageRisk(item) === "englishLikely", `Round 56: ${id} lost the evidence-versus-likely-English distinction`);
}
for (const id of [977, 930818, 1299, 1255, 958, 930637, 238, 305, 930708, 922, 930900]) {
  const item = r23ById(id);
  check(item && test.applicationLanguagePath(item).key === "english" && test.displayedScore(item) <= 8, `Round 56: ${id} lost its explicit-English gate or user-specific score ceiling`);
}
for (const id of [990001, 1026, 1061, 27]) {
  const item = r23ById(id);
  check(item && test.applicationLanguagePath(item).key === "spanish" && test.displayedScore(item) <= 5, `Round 56: ${id} lost its explicit-Spanish gate or user-specific score ceiling`);
}
for (const id of [930818, 37, 1299, 1255, 958, 930829, 382, 1287, 930843]) {
  check(test.experienceInfo(r23ById(id)).key === "intern", `Round 56: ${id} lost its internship/convenio feasibility penalty`);
}
check(test.experienceInfo(r23ById(1002)).key === "unknown" && test.experienceInfo(r23ById(12)).key === "unknown", "Round 56: roles without a stated experience threshold received an invented experience level");
check(round54EquipoSingular && test.applicationStatus(round54EquipoSingular).key === "closed" && !r41Current.some((item) => Number(item.id) === 930901), "Round 55: duplicate Equipo Singular LinkedIn mirror still counts as an independent current vacancy");
check(round55EquipoSingularCanonical && test.applicationStatus(round55EquipoSingularCanonical).key === "live" && test.locationBucket(round55EquipoSingularCanonical) === "barcelona" && test.applicationLanguagePath(round55EquipoSingularCanonical).key === "spanishLikely" && test.isInternshipRole(round55EquipoSingularCanonical) && test.displayedScore(round55EquipoSingularCanonical) === 0 && test.toLinks(round55EquipoSingularCanonical).some((url) => /talentclue\.com\/node\/127743936\/66909823/i.test(url)) && test.toLinks(round55EquipoSingularCanonical).some((url) => /4452389902/i.test(url)), "Round 55: canonical Equipo Singular internship lost its official route, mirror evidence, language risk or low-pay score");
check(round54OgilvySenior && test.applicationStatus(round54OgilvySenior).key === "live" && test.locationBucket(round54OgilvySenior) === "barcelona" && test.applicationLanguagePath(round54OgilvySenior).key === "spanishLikely" && test.experienceInfo(round54OgilvySenior).key === "senior" && test.displayedScore(round54OgilvySenior) === 0 && test.toLinks(round54OgilvySenior).some((url) => /4721164005/i.test(url)), "Round 54: Ogilvy Senior Art Director lost its official route, Spanish-likely gate, seniority or score floor");
check([317, 454].every((id) => test.applicationStatus(r23ById(id)).key === "closed" && !r41Current.some((item) => Number(item.id) === id)), "Round 55: an official-board removal or stale commission body still leaks into the current board");
for (const id of [175, 930718, 889, 860, 930869, 1303, 868]) {
  const item = r23ById(id);
  check(item && test.applicationLanguagePath(item).key === "spanish" && test.displayedScore(item) <= 5, `Round 55: ${id} lost its explicit Spanish/English compound gate or user-specific score ceiling`);
}
check(test.applicationLanguagePath(r23ById(1301)).key === "foreign" && test.displayedScore(r23ById(1301)) <= 4, "Round 55: The Colour Monster Catalan/Spanish/English compound gate is not fully penalized");
check(test.toLinks(r23ById(1227))[0]?.includes("videografo-especialista-en-edicion-de-videos-290359"), "Round 55: Natulim current canonical requisition was not repaired");
check(test.toLinks(r23ById(1293))[0]?.includes("16563075-disenador-grafico-y-creativo-freelance-social-media"), "Round 55: Bonita current Join requisition was not repaired");
check(test.applicationStatus(r23ById(930842)).key === "live" && test.applicationLanguagePath(r23ById(930842)).key === "english" && test.experienceInfo(r23ById(930842)).key === "senior" && test.displayedScore(r23ById(930842)) <= 8 && test.toLinks(r23ById(930842)).some((url) => /4452453717/i.test(url)), "Round 55: Voodoo current official role lost its employer mirror, English gate, seniority or score cap");
check(test.CURATED[930812]?.latestAuditSection === round56Section && test.CURATED[930899]?.latestAuditSection === round74Section, "Round 52/56/74: Skyscanner or CRAZE provenance was lost");
const round51AuditedItems = [...test.SCORE_LANGUAGE_RISK_OVERRIDES.entries()].map(([id, risk]) => ({ item: r23ById(id), risk }));
...
check(round51AuditedItems.length === 72 && round51Mismatches.length === 0, `Round 79: the 72 evidence-versus-likely-language risk corrections were not preserved after later source audits ${JSON.stringify({ total: round51AuditedItems.length, mismatches: round51Mismatches })}`);
check(test.CURATED[930898]?.latestAuditSection === round50Section && test.CURATED[427]?.latestAuditSection === round73Section, "Round 50/73: recovery provenance or current closure audit was lost");
check([...round49LikelySpanishIds.filter((id) => ![105, 886, 930843, 930829, 1257, 1258, 382, 1237, 877, 876, 867, 930876].includes(id)), 930897].every((id) => test.CURATED[id]?.latestAuditSection === round49Section) && [105, 886].every((id) => test.CURATED[id]?.latestAuditSection === round54Section) && [930829, 1257, 1258, 382].every((id) => test.CURATED[id]?.latestAuditSection === round56Section) && test.CURATED[1237]?.latestAuditSection === round66Section && test.CURATED[930843]?.latestAuditSection === round64Section && test.CURATED[877]?.latestAuditSection === round76Section && [876, 867, 930876].every((id) => test.CURATED[id]?.latestAuditSection === round59Section), "Round 49/56/59/64/66/76: likely-Spanish audit provenance was lost after the current recheck");
...
check(
  round50RevolutEmployerBranding &&
    ...
    test.applicationStatus(round50RevolutEmployerBranding).key === "closed" &&
    ...
    test.displayedScore(round50RevolutEmployerBranding) === 0 &&
    !r41Current.some((item) => Number(item.id) === 427),
  "Round 73: Revolut closure lost its Barcelona history, unknown-evidence, two-year or zero-score constraints",
);
check(
...
check(
  test.displayedScore(round50RevolutEmployerBranding) === 0 &&
    test.displayedScore(round50DragonsArtDirector) < test.displayedScore(r23ById(920)),
  "Round 73: a closed or foreign-language senior route outranks the user's Chinese/contact-first route",
);
...
check(
  round52Skyscanner &&
    ...
    test.displayedScore(round52Skyscanner) <= 10 &&
...
check(
  round52Craze &&
    ...
    test.displayedScore(round52Craze) === 8 &&
...
check(
  [778, 920, 24, 25].every((id, index) => {
    const sorted = r41Current.slice();
    test.sortRecords(sorted);
    return Number(sorted[index]?.id) === id;
  }) && test.displayedScore(round52Craze) < test.displayedScore(r23ById(25)),
  "Round 72: a pure English-language role displaced one of the four current Chinese/contact-first opportunities",
);
check(
  test.scoreLanguageRisk(r23ById(24)) === "chineseForeign" &&
    test.scoreLanguageRisk(r23ById(25)) === "chineseForeign" &&
    test.displayedScore(r23ById(24)) === 35.3 &&
    test.displayedScore(r23ById(25)) === 19.9,
  "Round 53: HKU or Tea Lab lost the Chinese-advantage plus foreign-language-gate scoring model",
);
check(
  test.applicationStatus(r23ById(930822)).key === "closed" &&
    r23ById(930822).tier === "X" &&
    !r41Current.some((item) => Number(item.id) === 930822),
  "Round 53: removed Dragons Junior Art Director 317709 still leaks into the current board",
);
check(
  test.applicationStatus(r23ById(930823)).key === "live" &&
    test.toLinks(r23ById(930823))[0]?.includes("marketing-paid-motion-designer-global-markets-316734") &&
    !test.toLinks(r23ById(930823)).some((url) => /306539/i.test(url)),
  "Round 53: Factorial Global Markets role lost its current canonical requisition",
);
check(test.CURATED[910]?.latestAuditSection === round76Section && test.CURATED[866]?.latestAuditSection === round57Section && test.CURATED[930814]?.latestAuditSection === round59Section, "Round 57/59/76: explicit-English corrections were not preserved after the next audit");
check(
...
check(
  round49Eurofirms?.postedAt === "2026-08-14" &&
    round49Eurofirms?.freshnessTag === "today" &&
  ...
  "Round 76: Eurofirms refreshed date, temporary contract or current source evidence regressed",
);
const round49Iconico = r23ById(1237);
check(
  test.applicationStatus(round49Iconico).key === "closed" &&
    round49Iconico.tier === "X" &&
  ...
  "Round 66: ICÓNICO expired full-time lead must stay in history with its exact Indeed and LinkedIn evidence",
);
...
check(
  test.CURATED[930884]?.latestAuditSection === "2026-08-13 Round 47 current-source and user-fit recheck" &&
    [930838, 172, 930891].every((id) => test.CURATED[id]?.latestAuditSection === round59Section) &&
    test.CURATED[930813]?.latestAuditSection === round79Section &&
    [930889, 930890].every((id) => test.CURATED[id]?.latestAuditSection === round58Section) &&
    test.CURATED[930874]?.latestAuditSection === round73Section &&
    test.CURATED[930892]?.latestAuditSection === round77Section,
  "Round 47/74/77: current-source corrections or new low-ranked opportunities were not preserved after the next audit",
);
...
check(r23Main.filter((item) => test.isChineseRelevant(item) && test.applicationStatus(item).key !== "closed").length === 5, "Round 72: Chinese-relevant current opportunity count must reflect the ELIM HOME closure at 5");
...
check(r47OgilvyLiquid && test.applicationStatus(r47OgilvyLiquid).key === "live" && test.applicationLanguagePath(r47OgilvyLiquid).key === "english" && test.displayedScore(r47OgilvyLiquid) === 6.9, "Round 73: Ogilvy Liquid Designer must retain the application-form English gate and motion-direction ceiling");
...
const r47ExpectedCurrent = new Map([
  [930889, { bucket: "barcelona", status: "live", language: "english", score: 5.8 }],
  [930890, { bucket: "barcelona", status: "live", language: "english", score: 3.2 }],
...
const r48EnglishCorrections = new Map([
  [910, { score: 7.2, location: "remote", evidence: /Fluent in English/i }],
  [866, { score: 4.7, location: "barcelona", evidence: /Excellent communication skills in English/i }],
  [930814, { score: 0, location: "barcelona", evidence: /Excellent written and spoken English is required/i }],
]);
for (const [id, expected] of r48EnglishCorrections) {
  ...
  check(test.displayedScore(item) === expected.score && test.displayedScore(item) <= 8, `Round 53: explicit-English score cap drifted for opportunity ${id}`);
...
check(
  ...
  "Round 66: THRU closed record still leaked into the current or priority ranking",
);
...
const r27Archived = [355, 1013, 1047, 1316, 1317, 574, 999, 338, 594, 571, 44, 98, 121, 179, 222, 250, 353, 530, 941, 1052, 1282, 1284, 1313, 198, 366, 392, 570, 998, 219, 53, 282, 285, 1251, 50, 967];
...
check(test.MY_OPPORTUNITY_SET.has(17) && test.applicationStatus(r23ById(17)).key === "live" && test.displayedScore(r23ById(17)) === 0, "Round 74: Neo live restoration or hard native-Russian feasibility gate regressed");
